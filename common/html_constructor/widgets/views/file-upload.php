...
<div class="form-group" id="<?= $model->id ?>">
	...
	<?php endif; ?>
	<input type="file" class="file_upload" name="" value="" <?= $multiple ? 'multiple' : '' ?> data-file_target_id="<?= $model->id ?>">
	<?php if (!empty($lastFiles)) : ?>
		<div class="box box-default collapsed-box" style="margin:10px 0">
			...
			<div class="box-body" style="display: none;">
				<?php
				foreach ($lastFiles as $media) : ?>
					<a href="#<?= $model->id ?>" data-last-file=<?= $media->id ?> data-attach=<?= $model->id ?>>
